...
void ExplodingSprite::reset(const Vector2f& pos){
	std::list<Chunk>::iterator iter = freeList.begin();
	std::vector<Frame*>::iterator frameIter = frames.begin();
    while(iter != freeList.end()){
		iter -> setPosition(pos + Vector2f((*frameIter) -> getSourceX(), (*frameIter) -> getSourceY()));
		iter -> reset();
        chunks.push_back(*iter);
        ++iter;
		++frameIter;
    }
	freeList.clear();
