...
FrameFactory::~FrameFactory() {
	std::map<std::string, SDL_Surface*>::iterator surface = surfaces.begin();
...
Frame* FrameFactory::getFrame(const std::string& name, double zoomX, double zoomY) {
  ...
  if ( pos == frames.end() ) {
	...
	SDL_Surface* temp = surface;
   	surface = zoomSurface(surface, zoomX, zoomY, SMOOTHING_ON);
...
std::vector<Frame*> FrameFactory::getFrames(const std::string& name, double zoomX, double zoomY) {
  ...
  for (unsigned i = 0; i < numberOfFrames; ++i) {
   ...
   surf = zoomSurface(surf, zoomX, zoomY, SMOOTHING_ON);
   SDL_FreeSurface(temp);
